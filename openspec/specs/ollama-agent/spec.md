...
Implements the Rust-side agentic loop that connects the chat panel to a locally running Ollama instance. Manages conversation history, calls the Ollama `/api/chat` endpoint, handles tool call detection and user approval, executes approved terminal commands, and emits Tauri events to drive the chat UI.
