# Spec: SSH Connect Skill

## Purpose

Defines the behavior of the SSH connect agent skill, which extracts a hostname and username from the user's message, resolves the hostname to an IP via the host-lookup script, and opens an SSH session in a terminal tab.
